...
#
# 用法：
#   update-dbip.sh -p <mmdb落地路径> [-s <systemd服务名>] [-y <YYYY-MM>]
#
# 参数：
#   -p PATH   mmdb 落地路径（必填，须与 server/.env 的 GEO_DB_PATH 一致）
#   -s NAME   后端 systemd 服务名（可选，默认跳过重启）
#   -y YM     指定月份，如 2026-09（可选，默认当前月）
#   -n        仅下载校验，不覆盖不重启（预演）
#
# 示例：
#   sudo update-dbip.sh -p /srv/geo/dbip-city-lite.mmdb -s blog
#   update-dbip.sh -p /srv/geo/dbip-city-lite.mmdb -n   # 预演，仅校验
#
# crontab（每月 1 号 3 点）：
#   0 3 1 * *  /path/to/update-dbip.sh -p /srv/geo/dbip-city-lite.mmdb -s blog >> /var/log/update-dbip.log 2>&1
...
if [ -z "${MMDB_PATH}" ]; then
  echo "[$(date)] 错误：必须用 -p 指定 mmdb 落地路径"
  usage
fi
