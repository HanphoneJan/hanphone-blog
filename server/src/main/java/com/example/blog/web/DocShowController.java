...
public class DocShowController {
    ...
    public Result<?> listDocs(
        ...
        if (page != null && size != null) {
            ...
            return new Result<>(true, StatusCode.OK, "获取文件列表成功", docService.listPublishedDoc(customPageable));
        }
        return new Result<>(true, StatusCode.OK, "获取文件列表成功", docService.listPublishedDoc());
    }
    ...
    public Result<Page<Doc>> hotDocs(
        ...
        return new Result<>(true, StatusCode.OK, "获取热门文件成功", docService.listHotDoc(pageable));
    }

    @GetMapping("/docs/recommended")
    public Result<List<Doc>> recommendedDocs(
            @RequestParam(defaultValue = "10") Integer size) {
        return new Result<>(true, StatusCode.OK, "获取推荐文件成功", docService.listRecommendDocTop(size));
    }

    @GetMapping("/docs/{docId}")
    public Result<Doc> getDocByDocId(@PathVariable String docId) {
        return new Result<>(true, StatusCode.OK, "获取文件成功", docService.getDocByDocId(docId));
    }
