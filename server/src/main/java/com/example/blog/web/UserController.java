...
public class UserController {
    ...
    // 绑定 OAuth 账号（需登录）
    @PostMapping("/user/bind/oauth")
    public Result<Map<String, Object>> bindOauth(@RequestBody Map<String, String> para,
                                                  HttpServletRequest request) {
        try {
            String token = request.getHeader("token");
            Long userId = Long.valueOf(para.get("userId"));
            String provider = para.get("provider");
            String providerId = para.get("providerId");

            if (userId == null || provider == null || providerId == null) {
                return new Result<>(false, StatusCode.ERROR, "参数不完整", null);
            }
            ...
            Map<String, Object> mergeResult = userService.bindOauth(userId, provider, providerId);

            if (mergeResult != null) {
                Map<String, Object> info = new HashMap<>();
                info.put("merged", true);
                info.put("mergedFromNickname", mergeResult.get("mergedFromNickname"));
                info.put("wasAdmin", mergeResult.get("wasAdmin"));
                return new Result<>(true, StatusCode.OK, provider + " 账号绑定成功，已合并历史账号数据", info);
            }

            return new Result<>(true, StatusCode.OK, provider + " 账号绑定成功", null);
        } catch (RuntimeException e) {
            return new Result<>(false, StatusCode.ERROR, e.getMessage(), null);
        } catch (Exception e) {
            return new Result<>(false, StatusCode.ERROR, "绑定失败", null);
        }
    }

    // 解绑 OAuth 账号（需登录）
    @PostMapping("/user/unbind/oauth")
    public Result<Void> unbindOauth(@RequestBody Map<String, String> para,
                                     HttpServletRequest request) {
        try {
            String token = request.getHeader("token");
            ...
            if (userIdStr == null || provider == null) {
                return new Result<>(false, StatusCode.ERROR, "参数不完整", null);
            }
            ...
            return new Result<>(true, StatusCode.OK, "解除" + provider + "绑定成功", null);
        } catch (RuntimeException e) {
            return new Result<>(false, StatusCode.ERROR, e.getMessage(), null);
        } catch (Exception e) {
            return new Result<>(false, StatusCode.ERROR, "解绑失败", null);
        }
    }
}