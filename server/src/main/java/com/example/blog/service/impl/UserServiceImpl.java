...
import com.example.blog.dao.UserMergeRepository;
...
public class UserServiceImpl implements UserService {
    ...
    private final UserMergeRepository userMergeRepository;

    public UserServiceImpl(UserRepository userRepository, UserMergeRepository userMergeRepository) {
        this.userRepository = Objects.requireNonNull(userRepository, "userRepository must not be null");
        this.userMergeRepository = Objects.requireNonNull(userMergeRepository, "userMergeRepository must not be null");
    }
    ...
    @Override
    @Transactional
    public Map<String, Object> bindOauth(Long currentUserId, String provider, String providerId) {
        ...
        // 如果当前用户已经绑定了该平台的不同账号，不允许覆盖
        String existingId = getProviderId(currentUser, provider);
        if (existingId != null && !existingId.equals(providerId)) {
            throw new RuntimeException("当前账号已绑定其他" + provider + "账号，请先解绑");
        }
        ...
        // 检查 providerId 是否已被占用
        User existingOauthUser = findUserByProviderId(provider, providerId);
        if (existingOauthUser == null) {
            // 正常绑定
            setProviderId(currentUser, provider, providerId);
        ...
        // 如果绑定在当前用户自己身上，直接返回
        if (existingOauthUser.getId().equals(currentUserId)) {
            return null;
        }

        // 账号合并：以 currentUser 为主，合并 existingOauthUser
        Long sourceId = existingOauthUser.getId();
        Long targetId = currentUser.getId();

        userMergeRepository.mergeBlogs(sourceId, targetId);
        userMergeRepository.mergeEssays(sourceId, targetId);
        userMergeRepository.mergeComments(sourceId, targetId);
        userMergeRepository.mergeEssayComments(sourceId, targetId);
        userMergeRepository.mergeBlogLikes(sourceId, targetId);
        userMergeRepository.mergeEssayLikes(sourceId, targetId);
        userMergeRepository.mergePrivateMessagesSent(sourceId, targetId);
        userMergeRepository.mergePrivateMessagesReceived(sourceId, targetId);

        Map<String, Object> result = new HashMap<>();
        result.put("mergedFromNickname", existingOauthUser.getNickname());
        result.put("wasAdmin", UserType.ADMIN.getCode().equals(existingOauthUser.getType()));

        userRepository.delete(existingOauthUser);
    ...
    public void unbindOauth(Long userId, String provider) {
        ...
        if ((user.getPassword() == null || user.getPassword().isEmpty())
            ...
            throw new RuntimeException("请先设置密码或邮箱，再解除绑定");
        }

        setProviderId(user, provider, null);
        user.setUpdateTime(new Date());
        userRepository.save(user);
    }

    private User findUserByProviderId(String provider, String providerId) {
        if ("github".equals(provider)) return userRepository.findByGithubId(providerId);
        if ("google".equals(provider)) return userRepository.findByGoogleId(providerId);
        throw new RuntimeException("Unknown provider: " + provider);
    }

    private String getProviderId(User user, String provider) {
        if ("github".equals(provider)) return user.getGithubId();
        if ("google".equals(provider)) return user.getGoogleId();
        throw new RuntimeException("Unknown provider: " + provider);
    }

    private void setProviderId(User user, String provider, String id) {
        if ("github".equals(provider)) user.setGithubId(id);
        else if ("google".equals(provider)) user.setGoogleId(id);
        else throw new RuntimeException("Unknown provider: " + provider);
    }
}