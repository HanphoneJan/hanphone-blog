...
;(async () => {
  ...
  try {
    ...
    // 定义需要加载的所有资源
    const waifuTipsUrl = live2d_path + 'waifu-tips.json'
    const cubism2Url = live2d_path + 'live2d.min.js'
    const cubism5Url = live2d_path + 'live2dcubismcore.min.js'

    // 定义图片资源 - 使用相对路径或从配置读取
    const basePath = live2d_path.replace(/\/$/, '')
    const imageUrls = [
      `${basePath}/models/mimi/mimi.4096/texture_00.png`,
      `${basePath}/models/37/37.4096/texture_00.png`,
      `${basePath}/models/37/37.4096/texture_01.png`
    ]
