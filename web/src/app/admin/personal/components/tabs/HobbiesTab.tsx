...
import { Heart, Plus, Trash2, Edit3 } from 'lucide-react'
...
interface HobbiesTabProps {
  ...
  loading: boolean
  onAdd: () => void
  onEdit: (hobby: Hobby, index: number) => void
  onDelete: (index: number, id: number) => void
}

export function HobbiesTab({ hobbies, loading, onAdd, onEdit, onDelete }: HobbiesTabProps) {
  return (
    <div className="bg-[rgb(var(--card))] backdrop-blur-sm rounded-b-xl shadow-sm border border-[rgb(var(--border))] border-t-0 overflow-hidden">
      <div className="py-3 px-6 border-b border-[rgb(var(--border))] flex justify-between items-center">
        <h2 className="text-lg font-semibold text-[rgb(var(--primary))] flex items-center">
          <Heart className="h-5 w-5 mr-2 text-[rgb(var(--primary))]" />
          爱好管理
        </h2>
        <button
          onClick={onAdd}
      ...
      <div className="p-2 lg:p-4 min-h-[90vh]">
        {loading ? (
          ...
          <div className="text-center py-10 text-[rgb(var(--text-muted))]">暂无爱好数据</div>
        ) : (
            ...
            {hobbies.map((hobby, index) => (
              ...
              >
                <button
                  onClick={e => {
                    e.stopPropagation()
                    onDelete(index, hobby.id)
                  }}
                  className="absolute top-2 right-2 bg-[rgb(var(--overlay))]/50 rounded-full p-1 text-slate-300 hover:text-red-500 transition-colors z-10"
                  aria-label="删除爱好"
                >
                ...
                <button
                  ...
                  className="absolute top-2 left-2 bg-[rgb(var(--overlay))]/50 rounded-full p-1 text-slate-300 hover:text-[rgb(var(--primary))] transition-colors z-10"
                  aria-label="编辑爱好"
                >
                ...
                <div className="p-3">
                  <div className="flex items-center mb-2">
                    <span className="mr-2 text-[rgb(var(--primary))]">{getIcon(hobby.icon_src)}</span>
