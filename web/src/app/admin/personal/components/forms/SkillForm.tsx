...
export function SkillForm({ formData, onChange }: SkillFormProps) {
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-sm text-[rgb(var(--text))] mb-1">技能名称</label>
        <input
          name="name"
          type="text"
          value={formData.name || ''}
      ...
      <div>
        <label className="block text-sm text-[rgb(var(--text))] mb-1">技能链接</label>
        <input
          name="url"
          type="url"
          value={formData.url || ''}
      ...
      <div>
        <label className="block text-sm text-[rgb(var(--text))] mb-1">技能类型</label>
        <input
          name="icon_src"
          type="text"
          ...
          onChange={onChange}
          className="w-full px-4 py-2 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card))] text-[rgb(var(--text))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--primary))] transition-all"
          placeholder="输入技能类型 (music/video/sport/literature/game/code)"
          required
        />
      </div>
      <div>
        <label className="block text-sm text-[rgb(var(--text))] mb-1">技能描述</label>
        <textarea
          name="desc"
          value={formData.desc || ''}
          onChange={onChange}
