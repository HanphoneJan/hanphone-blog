...
export default function FileManagementPage() {
    ...
    for (const [index, dir] of dirs.entries()) {
      ...
      breadcrumbs.push(
        <React.Fragment key={`breadcrumb-${index}`}>
          <ChevronRight className="h-3.5 w-3.5 mx-1 text-[rgb(var(--text-muted))] dark:text-[rgb(var(--text-muted))]" />
          <button
  ...
  const renderMobileFileItem = (item: FileItem) => (
    <div className="border-b border-[rgb(var(--border))]/30 p-3 last:border-0" style={{ borderColor: 'rgb(var(--border))' }}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center flex-1 min-w-0">
          {item.isDirectory ? (
            <Folder className="h-5 w-5 text-yellow-500 mr-2 shrink-0" />
          ) : (
            <File className="h-5 w-5 text-[rgb(var(--text-muted))] mr-2 shrink-0" />
          )}
          <button
        ...
        <span className="text-xs text-[rgb(var(--text-muted))] whitespace-nowrap ml-2" style={{ color: 'rgb(var(--text-muted))' }}>
          {formatDate(item.mtime, true)}
        </span>
      </div>

      <div className="flex justify-between items-center">
        <span className="text-xs text-[rgb(var(--text-muted))]" style={{ color: 'rgb(var(--text-muted))' }}>
    ...
    if (isImageFile(name)) {
      ...
      return (
        <div className="flex flex-col justify-center items-center h-full p-4">
          <div className="mb-4 text-[rgb(var(--text))] text-center">
            <File className="h-16 w-16 mx-auto mb-2 text-[rgb(var(--text-muted))]" />
  ...
  return (
    <div className="font-sans min-h-screen flex flex-col bg-[rgb(var(--bg))] text-[rgb(var(--text))] relative overflow-hidden" style={{ background: 'linear-gradient(to bottom, rgb(var(--bg)))', color: 'rgb(var(--text))' }}>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto lg:px-2 lg:py-2 relative z-10">
        <div className="bg-[rgb(var(--card))]/80 backdrop-blur-sm lg:rounded-xl shadow border border-[rgb(var(--border))] overflow-hidden">
          {/* 顶部导航 - 始终保持在一行 */}
          <div className="px-4 py-3 border-b border-[rgb(var(--border))] bg-[rgb(var(--muted))]/60">
            <div className="flex items-center justify-between gap-2 min-w-0">
              <h1 className="text-lg font-bold text-[rgb(var(--text))] truncate">文件管理系统</h1>
          ...
          <div className="px-4 pb-1 min-h-[90vh]">
              ...
              <div className="flex items-center whitespace-nowrap">
                ...
                {currentDir && (
                  <button
                    ...
                    className="text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text))] flex items-center transition-colors" style={{ color: 'rgb(var(--text-muted))' }}
                  >
            ...
            {/* 文件列表 */}
            {loading ? (
              <div className="bg-[rgb(var(--card))]/60 rounded-lg shadow overflow-hidden border border-[rgb(var(--border))]" style={{ backgroundColor: 'rgb(var(--card))', borderColor: 'rgb(var(--border))' }}>
                <div className="px-4 py-2 text-center text-[rgb(var(--text-muted))]" style={{ color: 'rgb(var(--text-muted))' }}>
                  <Loader2 className="animate-spin h-5 w-5 mx-auto mb-2 text-[rgb(var(--text-muted))]" />
                  加载中...
                </div>
              </div>
            ) : files.length === 0 ? (
              <div className="bg-[rgb(var(--card))]/60 rounded-lg shadow overflow-hidden border border-[rgb(var(--border))]" style={{ backgroundColor: 'rgb(var(--card))', borderColor: 'rgb(var(--border))' }}>
                <div className="p-4 text-center text-[rgb(var(--text-muted))]" style={{ color: 'rgb(var(--text-muted))' }}>当前目录为空</div>
              </div>
            ) : isMobile ? (
              // 移动设备视图 - 时间与名称水平对齐
              <div className="bg-[rgb(var(--card))]/60 rounded-lg shadow overflow-hidden border border-[rgb(var(--border))]" style={{ backgroundColor: 'rgb(var(--card))', borderColor: 'rgb(var(--border))' }}>
                ...
                <table className="min-w-full divide-y divide-slate-200/50" style={{ borderColor: 'rgb(var(--border))' }}>
                  <thead className="bg-[rgb(var(--muted))]/60">
                    <tr>
                      <th
                        scope="col"
                        className="px-4 py-2.5 text-left text-xs font-medium text-[rgb(var(--text))] uppercase tracking-wider"
                      >
                        名称
                      </th>
                      <th
                      ...
                      <th
                        scope="col"
                        className="px-4 py-2.5 text-right text-xs font-medium text-[rgb(var(--text))] uppercase tracking-wider"
                      >
                        操作
                      </th>
                    </tr>
                    ...
                    {files
                      ...
                      .map(item => (
                        ...
                        >
                          ...
                          <td
                            className={`px-4 py-3 whitespace-nowrap text-xs text-[rgb(var(--text-muted))] ${
                              isSmallScreen ? 'hidden md:table-cell' : ''
                            }`}
                          >
                            -
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap text-xs text-[rgb(var(--text-muted))]" style={{ color: 'rgb(var(--text-muted))' }}>
                    ...
                    {files
                      ...
                      .map(item => (
                        ...
                        >
                          <td className="px-4 py-3 whitespace-nowrap">
                            <div className="flex items-center">
                              <File className="h-4.5 w-4.5 text-[rgb(var(--text-muted))] mr-2" />
      ...
      {showCreateDirModal &&
        ...
        createPortal(
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            ...
            <div className="relative z-10 bg-[rgb(var(--card))]/90  rounded-xl shadow-2xl border border-[rgb(var(--border))] w-full max-w-md transform transition-transform duration-300 scale-100 animate-scaleIn" style={{ backgroundColor: 'rgb(var(--card))', borderColor: 'rgb(var(--border))' }}>
              <div className="p-4 border-b border-[rgb(var(--border))] flex justify-between items-center bg-[rgb(var(--muted))]/60 rounded-t-xl" style={{ borderColor: 'rgb(var(--border))', backgroundColor: 'rgb(var(--muted))' }}>
                <h3 className="text-base font-semibold text-[rgb(var(--text))] flex items-center gap-2">
                ...
                <button
                  ...
                  className="text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text))] transition-colors p-1 rounded-full hover:bg-[rgb(var(--hover))]/50"
                >
              ...
              <div className="p-4">
                <div className="mb-3">
                  <label className="block text-xs font-medium text-[rgb(var(--text))] mb-1">目录名称</label>
                  <input
                    type="text"
                    value={newDirName}
                    onChange={e => setNewDirName(e.target.value)}
                    className="w-full px-3 py-2 border border-[rgb(var(--border))] bg-[rgb(var(--card))]/60 text-[rgb(var(--text))] rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all text-sm"
                    placeholder="请输入目录名称"
                    autoFocus
                  />
                  {error && (
              ...
              <div className="p-3 border-t border-[rgb(var(--border))] flex justify-end gap-2 bg-[rgb(var(--muted))]/60 rounded-b-xl" style={{ borderColor: 'rgb(var(--border))', backgroundColor: 'rgb(var(--muted))' }}>
                <button
                  onClick={() => {
                    setShowCreateDirModal(false)
                    setNewDirName('')
                  ...
                  className="px-3 py-1.5 rounded-lg bg-[rgb(var(--muted))]/60 hover:bg-[rgb(var(--muted))] text-[rgb(var(--text))] transition-all duration-200 text-sm"
                >
                  取消
                </button>
                <button
      ...
      {previewModalVisible &&
        ...
        createPortal(
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            ...
            <div className="relative z-10 bg-[rgb(var(--card))]/90  rounded-xl shadow-2xl border border-[rgb(var(--border))] w-full h-full max-w-5xl max-h-[90vh] transform transition-transform duration-300 scale-100 animate-scaleIn flex flex-col" style={{ backgroundColor: 'rgb(var(--card))', borderColor: 'rgb(var(--border))' }}>
              <div className="p-4 border-b border-[rgb(var(--border))] flex justify-between items-center bg-[rgb(var(--muted))]/60 rounded-t-xl" style={{ borderColor: 'rgb(var(--border))', backgroundColor: 'rgb(var(--muted))' }}>
                <h3 className="text-base font-semibold text-[rgb(var(--text))] flex items-center gap-2">
      ...
      {uploading &&
        ...
        createPortal(
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            <ModalOverlay />
            <div className="relative z-10 bg-[rgb(var(--card))]/90  rounded-xl shadow-2xl border border-[rgb(var(--border))] w-full max-w-md transform transition-transform duration-300 scale-100 animate-scaleIn p-4 text-center" style={{ backgroundColor: 'rgb(var(--card))', borderColor: 'rgb(var(--border))' }}>
              ...
              <p className="text-[rgb(var(--text))] text-sm">文件上传中，请稍候...</p>
            </div>
          </div>,
          document.body
        )}
