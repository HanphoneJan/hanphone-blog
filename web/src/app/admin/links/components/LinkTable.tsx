          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex-shrink-0">
              {link.editingAvatar ? (
                <div className="flex items-center gap-1">
                  <input
                    type="text"
                    value={getLocalValue(link.id!, 'avatar')}
                    onChange={e => onLocalInputChange(link.id!, 'avatar', e.target.value)}
                    placeholder="头像URL"
                    className="w-32 px-2 py-1 text-xs rounded border border-[rgb(var(--border))] bg-[rgb(var(--bg))] focus:outline-none focus:ring-1 focus:ring-[rgb(var(--primary))]"
                  />
                  <button onClick={() => onSaveAvatar(link.id)} className="p-1 text-green-500 hover:bg-green-50 rounded">
                    <Check className="w-3 h-3" />
                  </button>
                  <button onClick={() => onCancelEditAvatar(link.id)} className="p-1 text-red-500 hover:bg-red-50 rounded">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ) : (
                <div onClick={() => onEditAvatar(link.id)} className="cursor-pointer" title="点击编辑头像">
                  {renderImagePreview(link.avatar, link.name || '头像', 'sm')}
                </div>
              )}
            </div>
              ...
              {link.name ? (
                ...
                <div className="flex items-center gap-2">
                  <span className="text-yellow-600 dark:text-yellow-400 text-xs font-medium">
                    {link.applyText.slice(0, 20)}{link.applyText.length > 20 ? '...' : ''}
                  </span>
                  <span className="text-[rgb(var(--muted))] text-[10px]">(待解析)</span>
                </div>
              ) : (
                <span className="text-[rgb(var(--muted))] text-xs italic">未命名</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              {link.url ? (
                ...
                <span className="p-1.5 rounded text-[rgb(var(--muted))]/30 cursor-not-allowed" title="暂无网址">
                  <ExternalLink className="w-4 h-4" />
                </span>
              )}
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs">
            ...
            <div className="flex items-center gap-1">
              ...
              {renderEditableField(
                link,
                'rss',
                link.editingRss || false,
                () => onEditRss(link.id),
                () => onSaveRss(link.id),
                () => onCancelEditRss(link.id),
                link.rss || '',
                '-'
              )}
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              {link.editingSiteshot ? (
                <div className="flex items-center gap-1">
                  <input
                    type="text"
                    value={getLocalValue(link.id!, 'siteshot')}
                    onChange={e => onLocalInputChange(link.id!, 'siteshot', e.target.value)}
                    placeholder="截图URL"
                    className="w-32 px-2 py-1 text-xs rounded border border-[rgb(var(--border))] bg-[rgb(var(--bg))] focus:outline-none focus:ring-1 focus:ring-[rgb(var(--primary))]"
                  />
                  <button onClick={() => onSaveSiteshot(link.id)} className="p-1 text-green-500 hover:bg-green-50 rounded">
                    <Check className="w-3 h-3" />
                  </button>
                  <button onClick={() => onCancelEditSiteshot(link.id)} className="p-1 text-red-500 hover:bg-red-50 rounded">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ) : (
                <div onClick={() => onEditSiteshot(link.id)} className="cursor-pointer" title="点击编辑截图">
                  {link.siteshot ? renderImagePreview(link.siteshot, '站点截图', 'md') : (
            ...
            <div className="flex items-center gap-1">
              ...
              {link.editingColor ? (
                <div className="flex items-center gap-1">
                  <input
                    type="text"
                    value={getLocalValue(link.id!, 'color')}
                    onChange={e => onLocalInputChange(link.id!, 'color', e.target.value)}
                    placeholder="#1890ff"
                    className="w-20 px-2 py-1 text-xs rounded border border-[rgb(var(--border))] bg-[rgb(var(--bg))] focus:outline-none focus:ring-1 focus:ring-[rgb(var(--primary))]"
                  />
                  <button onClick={() => onSaveColor(link.id)} className="p-1 text-green-500 hover:bg-green-50 rounded">
                    <Check className="w-3 h-3" />
                  </button>
                  <button onClick={() => onCancelEditColor(link.id)} className="p-1 text-red-500 hover:bg-red-50 rounded">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ) : (
                <div
                  onClick={() => onEditColor(link.id)}
                  className="w-4 h-4 rounded border border-[rgb(var(--border))] cursor-pointer hover:scale-110 transition-transform"
                  style={{ backgroundColor: link.color || '#1890ff' }}
                  title={`${link.color} (点击编辑)`}
                />
              )}
            </div>
