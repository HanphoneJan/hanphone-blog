...
export function useLinks() {
  ...
  // 解析applyText（纯前端解析）
  const parseApplyTextFn = async (friendLink: FriendLink): Promise<ParsedApplyText | null> => {
    ...
    try {
      ...
      const result = parseApplyText(friendLink.applyText)

      if (Object.keys(result).length > 0) {
        showAlert('解析成功')
        return result
      } else {
        showAlert('未能从文本中解析出有效字段')
        return null
      }
    } catch (error) {
  ...
  return {
    ...
    toggleRecommend,
    togglePublished,
    parseApplyText: parseApplyTextFn,
    applyParsedData,
    updateLocalList
  }
}
