...
export default function TagsManagementPage() {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="min-h-screen flex flex-col bg-gradient-to-b from-blue-50 via-white to-blue-50 text-[rgb(var(--text))] relative overflow-hidden"
    ...
    >
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto lg:px-2 lg:py-2 relative z-10">
        ...
        <motion.div
          ...
          className="bg-[rgb(var(--card))]/80 backdrop-blur-sm lg:rounded-xl shadow-sm min-h-[100vh] border border-[rgb(var(--border))] overflow-hidden"
        >
          ...
          <div className="py-2 px-6 border-b border-[rgb(var(--border))] grid grid-cols-[auto_1fr_auto] items-center gap-4">
            ...
            <div className="relative min-w-[80px]">
              ...
              <input
                ...
                placeholder="搜索..."
                className="pl-10 pr-4 py-2 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card))]/60 text-[rgb(var(--text))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--primary))] transition-all w-full text-sm"
              />
            </div>
            ...
            {loading ? (
              ...
              <div className="space-y-4 h-[400px] flex flex-col justify-center">
                {[1, 2, 3, 4, 5, 6].map(item => (
                  <div key={item} className="animate-pulse bg-[rgb(var(--muted))]/50 rounded-lg h-24"></div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
                {filteredTagList.map(tag => (
                  <motion.div
                    ...
                    className="bg-[rgb(var(--card))]/60 border border-[rgb(var(--border))] rounded-lg px-3 py-2 lg:p-4 hover:bg-[rgb(var(--hover))]/80 transition-all duration-150 hover:shadow-md hover:border-[rgb(var(--primary))]/30 relative group"
                  >
                    {/* 操作按钮（悬停显示） */}
                    <div className="absolute right-2 top-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => handleOpenDialog(tag)}
                        className="p-1.5 rounded-full text-[rgb(var(--text-muted))] hover:text-blue-600 hover:bg-blue-500/10 transition-colors"
                        title="编辑"
                      >
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-[rgb(var(--text))] mb-1">标签名称</label>
                  <input
                    type="text"
                    value={formName}
                    onChange={e => setFormName(e.target.value)}
                    className="w-full px-4 py-2 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card))]/60 text-[rgb(var(--text))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--primary))] transition-all"
                    placeholder="请输入标签名称"
                  />
                </div>
              </div>
