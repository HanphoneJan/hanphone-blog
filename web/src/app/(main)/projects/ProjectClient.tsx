...
export default function ProjectClient({ initialProjects }: ProjectClientProps) {
  ...
  const highlightText = (text: string, query: string) => {
    ...
    return (
      <span>
        {before}
        <span className="bg-[rgb(var(--primary)/0.3)] text-[rgb(var(--primary))] px-0.5 rounded">
          {match}
        </span>
    ...
    if (typeConfig.layout === 'large') {
      return (
        <motion.div
          ...
          animate="visible"
          whileHover="hover"
          className="project-card bg-[rgb(var(--card))] backdrop-blur-sm rounded-xl shadow-sm overflow-hidden border border-[rgb(var(--border))] hover:shadow-lg hover:border-[rgb(var(--primary)/0.5)] group "
        >
                ...
                <AnimatePresence>
                  {project.recommend && (
                    <motion.div
                      ...
                      className="absolute top-2 right-2 bg-[rgb(var(--primary))] text-white px-2.5 py-1 rounded-full flex items-center shadow-lg"
                    >
