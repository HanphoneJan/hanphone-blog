...
const MsgTextarea = forwardRef<HTMLTextAreaElement, MsgTextareaProps>(({
  value,
  onChange,
  placeholder = '',
  rows = 2,
...
MsgTextarea.displayName = 'MsgTextarea'
...
export default function MessageClient() {
  ...
  // textarea refs for emoji picker
  const messageTextareaRef = useRef<HTMLTextAreaElement>(null)
  const replyTextareaRefs = useRef<Map<number, HTMLTextAreaElement>>(new Map())
          ...
          <div className="flex-1 min-w-0">
            ...
            {isReplying && (
              <div className="msg-reply-box">
                <MsgTextarea
                  ref={(el) => {
                    if (el) replyTextareaRefs.current.set(message.id, el)
                    else replyTextareaRefs.current.delete(message.id)
                  }}
                ...
                <div className="flex justify-between items-center mt-2">
                  <EmojiPicker getTextarea={() => replyTextareaRefs.current.get(message.id) || null} />
          ...
          <div className="flex gap-2 items-center mt-1">
            {!isMobile ? (
              <div className="w-11 flex justify-center shrink-0">
                <EmojiPicker getTextarea={() => messageTextareaRef.current} />
              </div>
            ) : (
              <EmojiPicker getTextarea={() => messageTextareaRef.current} />
            )}
