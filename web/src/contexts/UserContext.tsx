...
import { TIME, STORAGE_KEYS, USER_TYPE } from '@/lib/constants'
...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const isTokenExpired = (expireTime: string | null): boolean => {
    ...
    try {
      ...
      return expireDate.getTime() - now.getTime() < TIME.TOKEN_EXPIRE_THRESHOLD
    } catch (error) {
  ...
  useEffect(() => {
    try {
      ...
      const storedExpire = localStorage.getItem(STORAGE_KEYS.EXPIRE)
      ...
      if (storedUser) {
        ...
        // 根据用户类型设置管理员状态
        setAdministrator(parsedUser.type === USER_TYPE.ADMIN_STRING)
      }
  ...
  useEffect(() => {
    if (userInfo) {
      setAdministrator(userInfo.type === USER_TYPE.ADMIN_STRING)
    } else {
  ...
  useEffect(() => {
    if (expire) {
      localStorage.setItem(STORAGE_KEYS.EXPIRE, JSON.stringify(expire))
    } else {
      localStorage.removeItem(STORAGE_KEYS.EXPIRE)
    }
  }, [expire])
  ...
  const onLogout = () => {
    ...
    setAdministrator(false)
    // admin 路由退出后跳转首页，非 admin 路由保持在当前页面
