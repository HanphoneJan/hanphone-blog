...
import { showMessage, setMessageHandler, clearMessageTimer, welcomeMessage, getPageSpecificDefaultMessages, setModelReady } from '../message';
import { getLocalStorage, setLocalStorage, randomSelection } from '../utils';
import logger from '../logger';
import { MessageCircle, User, X, Camera } from 'lucide-react';
...
export function Live2DContainer({ config, tips, models, onClose }: Live2DContainerProps) {
  ...
  useEffect(() => {
    ...
    const messageArray = [...tips.message.default, ...getPageSpecificDefaultMessages()];
    ...
    // 鼠标悬停事件 - 使用冷却 Map 替代永久阻塞
    const hoverCooldowns = new Map<string, number>();
    const firedElements = new Set<string>();

    // 获取元素唯一标识（用于 oncePerElement 去重）
    const getElementIdentifier = (el: Element, selector: string): string => {
      const linkEl = el.querySelector('a[href]') || el.closest('a[href]');
      const href = linkEl?.getAttribute('href') || '';
      const id = (el as HTMLElement)?.id || '';
      return `${selector}:${href || id || el.textContent?.trim().slice(0, 30)}`;
    };

    const handleMouseOver = (event: MouseEvent) => {
      for (const { selector, text, pages, dynamicText, oncePerElement } of tips.mouseover) {
        // 页面路由过滤
        if (pages && !pages.some(p => {
          try { return new RegExp(p).test(window.location.pathname); }
          catch { return window.location.pathname === p; }
        })) continue;

        if (!(event.target as HTMLElement)?.closest(selector)) continue;

        const now = Date.now();
        const lastTriggered = hoverCooldowns.get(selector) || 0;
        if (now - lastTriggered < 10000) return;

        // oncePerElement 去重
        if (oncePerElement) {
          const matchedEl = (event.target as HTMLElement)?.closest(selector);
          if (matchedEl) {
            const elId = getElementIdentifier(matchedEl, selector);
            if (firedElements.has(elId)) return;
          }
        }

        // 动态文本优先，回退到静态文本
        const matchedEl = (event.target as HTMLElement)?.closest(selector);
        const finalText = dynamicText && matchedEl ? dynamicText(matchedEl) : text;
        if (!finalText) continue;

        const msg = randomSelection(finalText).replace('{text}', (event.target as HTMLElement).innerText);
        if (showMessage(msg, 4000, 8)) {
          hoverCooldowns.set(selector, now);
          if (oncePerElement && matchedEl) {
            firedElements.add(getElementIdentifier(matchedEl, selector));
          }
        }
        return;
      }
    ...
    const handleFocusIn = (event: FocusEvent) => {
      ...
      if (!target.matches('.comment-textarea, textarea.msg-textarea')) return;
      const msgs = tips.message.commentFocus;
      if (msgs && msgs.length > 0) {
        showMessage(msgs, 4000, 7);
      }
    };
    ...
    if (window.location.pathname.match(/^\/blog\/[^/]+$/)) {
      ...
      const scrollTimer = setTimeout(() => {
        ...
        const scrollMessages = tips.message.scrollDepth || [];
        if (scrollMessages.length === 0) return;

        const thresholds = [0.3, 0.7, 1.0];
        const handleScroll = () => {
          ...
          thresholds.forEach((threshold, idx) => {
            if (progress >= threshold && !firedDepths.has(idx)) {
              firedDepths.add(idx);
              const msg = scrollMessages[idx];
              if (msg) showMessage(msg, 5000, 7);
            }
          });
        };
      ...
      scrollCleanup = () => clearTimeout(scrollTimer);
    }
  ...
  return (
    <div
      ref={elementRef}
      className={`fixed z-[9999] duration-500 ease-out ${
        isVisible && isEntered ? 'opacity-100' : 'opacity-0 pointer-events-none'
      }`}
      style={{
        left: position.x,
        top: position.y,
    ...
    >
      ...
      {/* 工具栏 - 移到模型外部右侧偏下 */}
      <div className="absolute -right-6 bottom-24 flex flex-col gap-1 opacity-50 hover:opacity-100 transition-opacity duration-300">
        {tools.map((tool) => (
          <button
            key={tool.id}
            onClick={(e) => {
