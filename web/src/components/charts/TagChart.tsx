...
const TagChart: React.FC<TagChartProps> = ({ style, rotationSpeed = 0.3 }) => {
  ...
  const getTagColors = () => {
    switch (theme) {
      case 'light':
        return ['#3b82f6', '#10b981', '#f59e0b', '#ec4899', '#8b5cf6', '#6366f1']
      case 'dark':
        return ['#3b82f6', '#10b981', '#f59e0b', '#ec4899', '#8b5cf6', '#6366f1']
      case 'macaron':
        return ['#ec4899', '#a855f7', '#f43f5e', '#db2777', '#f472b6', '#e879f9']
      case 'cyber':
        return ['#06b6d4', '#22d3ee', '#0891b2', '#67e8f9', '#06b6d4', '#22d3ee']
      default:
        return ['#3b82f6', '#10b981', '#f59e0b', '#ec4899', '#8b5cf6', '#6366f1']
    }
  }
  ...
  }, [rotationSpeed, theme]); // 添加rotationSpeed和theme依赖
      ...
      >
        {renderList.map(tag => {
          ...
          const getHoverBg = () => {
            switch (theme) {
              case 'light':
                return 'hover:bg-slate-800/10'
              case 'dark':
                return 'hover:bg-white/10'
              case 'macaron':
                return 'hover:bg-purple-900/10'
              case 'cyber':
          ...
          return (
            <div
              key={tag.id}
              className={`absolute transition-all duration-300 cursor-pointer px-2 py-1 rounded-md whitespace-nowrap ${getHoverBg()}`}
              style={{
                left: `${left}px`,
                top: `${top}px`,
                color: tag.color,
