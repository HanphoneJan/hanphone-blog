...
每次推送或 PR 到 `main` 或 `develop` 分支时，会自动运行：

1. **Server Tests**: Maven 单元测试
2. **Web Unit Tests**: Vitest 单元测试 + ESLint 检查
3. **Web E2E Tests**: Playwright 端到端测试
4. **Build Verification**: 服务端打包 + 前端构建

### 测试结果

- 测试报告会自动上传到 GitHub Artifacts
- Playwright 报告保留 7 天
